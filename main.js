...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.location.hash === '#audit') {
    ...
    const auditNote = document.querySelector('#audit-context-note');
    if (serviceSelect) {
    ...
    if (auditNote) auditNote.style.display = 'flex';
  }
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      if (!emailPattern.test(email)) {
        if (status) {
          status.style.color = 'var(--danger)';
          status.textContent = 'That email address doesn\'t look complete — please double-check it.';
        }
